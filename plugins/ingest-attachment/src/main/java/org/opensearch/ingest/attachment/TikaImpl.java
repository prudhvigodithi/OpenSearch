    ...
    static {
        ...
        FontMappers.set(new FontMapper() {
            ...
            public FontMapping<FontBoxFont> getFontBoxFont(String baseFont, PDFontDescriptor fd) {
                try (InputStream in = TikaImpl.class.getResourceAsStream("/fonts/n019003l.pfb")) {
                    if (in == null) return new FontMapping<>(null, true);
                    Type1Font t1 = Type1Font.createWithPFB(in);
                    return new FontMapping<>(t1, true);
                } catch (IOException e) {
            ...
            public FontMapping<TrueTypeFont> getTrueTypeFont(String baseFont, PDFontDescriptor fd) {
                try (InputStream in = TikaImpl.class.getResourceAsStream("/fonts/DroidSansFallback.ttf")) {
                    TrueTypeFont ttf = new TTFParser(false).parse(new RandomAccessReadBuffer(in.readAllBytes()));
            ...
            public CIDFontMapping getCIDFont(String baseFont, PDFontDescriptor fd, PDCIDSystemInfo cid) {
                // No CID substitutions from the OS either; signal "no mapping".
